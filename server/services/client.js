  ...
  init() {
    ...
    const setRedisData = async ({ key, data, expiredInSeconds = 0 }) => {
      ...
      if (expiredInSeconds > 0) {
        return redis.setex(newKey, expiredInSeconds, data);
      }
    ...
    const increment = async ({ key, expiredInSeconds = 0 }) =>
      new Promise((resolve, reject) => {
        const newKey = `${strapi.redis.prefix}:${key}`;
    ...
    const getTimeoutIncrementKey = async (key) =>
      new Promise((resolve, reject) => {
        const newKey = `${strapi.redis.prefix}:${key}`;
    ...
    const getUniqueProcess = async ({ key, expiredInSeconds = 0 }) =>
      new Promise((resolve, reject) => {
        const newKey = `${strapi.redis.prefix}:${key}`;
